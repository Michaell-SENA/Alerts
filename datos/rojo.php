...
<?php 
    ...
    if (isset($_GET['nombre'])) 
    {
        ...
        $query = "SELECT t1.causa_reporte_aprendiz, t1.nombre, t1.apellido, t2.nombre_doc, t1.num_doc, t1.telefono, t1.direccion, t5.nombre_nivel, t1.programa, t1.ficha, t3.nombre_jornada, t7.nombre_sede, t4.nombre_mot_reporte, t6.nombre_reporte, t1.accion, t1.documento_soporte, t1.nombre_ins, t1.apellido_ins, t1.telefono_ins, t1.correo_ins FROM obj_alerta AS t1, obj_doc_sena AS t2, obj_jornada_sena AS t3, obj_mot_reporte_sena AS t4, obj_nivel_forma_sena AS t5, obj_per_reporte_sena AS t6, obj_sede_sena AS  t7 WHERE t1.responsable = '$nombreS' AND t2.id_obj_doc_sena = t1.doc AND t5.id_obj_nivel_sena = t1.nivel AND t1.jornada = t3.id_obj_jornada_sena AND t1.sede = t7.id_obj_sede_sena AND t1.cusa_reporte = t4.id_obj_mot_reporte_sena AND t1.reporte_diri = t6.id_obj_per_reporte_sena AND t1.estado = 'ROJO' ORDER BY t1.id_obj_alerta ASC LIMIT $desde,$por_pagina;";
        ...
        <table class='table table-bordered' id='rojo'>
            ...
            <thead>
                <tr>
                    <th class='rojo'>NOMBRE APRENDIZ</th>
                    <th class='rojo'>APELLIDO APRENDIZ</th>
                    <th class='rojo'>TIPO DOC</th>
                    <th class='rojo'>DOCUMENTO</th>
                    <th class='rojo'>TELEFONO</th>
                    <th class='rojo'>NIVEL</th>
                    <th class='rojo'>PROGRAMA</th>
                    <th class='rojo'>FICHA</th>
                    <th class='rojo'>JORNADA</th>
                    <th class='rojo'>SEDE</th>
                </tr>
            </thead>
            <tbody>";
            ...
            {                                            
                ...
                $parte1 .='
                <tr>
                    <td class=rojo>'.$filas['nombre'].'</td>
                    <td class=rojo>'.$filas['apellido'].'</td>
                    <td class=rojo>'.$filas['nombre_doc'].'</td>
                    <td class=rojo>'.$filas['num_doc'].'</td>
                    <td class=rojo>'.$filas['telefono'].'</td>
                    <td class=rojo>'.$filas['nombre_nivel'].'</td>
                    <td class=rojo>'.$filas['programa'].'</td>
                    <td class=rojo>'.$filas['ficha'].'</td>
                    <td class=rojo>'.$filas['nombre_jornada'].'</td>
                    <td class=rojo>'.$filas['nombre_sede'].'</td>
                </tr>';
            }
            echo $parte1 .="
            </tbody>
